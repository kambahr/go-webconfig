...
func (c *Config) getConfigLeaves(lines []string, i int, keys []string) int {
	...
	for {
		...
		} else if strings.HasPrefix(l, "display-mode") {
		...
		} else if strings.HasPrefix(l, "seconds-to-display") {
			s := c.parseCofigLine(l, "seconds-to-display")
			c.MessageBanner.SecondsToDisplay, _ = strconv.Atoi(s)

		} else if strings.HasPrefix(l, "allowed-methods") {
		...
		} else if strings.HasPrefix(l, "forward-paths") {
